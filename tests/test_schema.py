...
class SchemaTest(unittest.TestCase):
    ...
    def test_create_image(self):
        ...
        executed = self.create_image(
            image_str="image", image_type=ImageType.identity, user_id=user_id
        )
        assert executed == {
            "data": OrderedDict(
                [
    ...
    def test_verify_user(self):
        ...
        executed = self.create_image(
            image_str=image_str, image_type=ImageType.identity, user_id=user_id
        )
        assert executed == {
            "data": OrderedDict(
                [
        ...
        image_path = (
            os.path.dirname(os.path.realpath(__file__)) + "/resources/selfie.jpg"
        )
        with open(image_path, "rb") as imageFile:
            image_str = base64.b64encode(imageFile.read()).decode("utf-8")

        executed = self.create_image(
            image_str=image_str, image_type=ImageType.profile, user_id=user_id
        )
        assert executed == {
            "data": OrderedDict(
                [
        ...
        verification_schema = graphene.Schema(
            query=VerifyQuery, mutation=VerifyMutation
        )
        verification_client = Client(verification_schema)
        executed = verification_client.execute(
            """mutation {
        ...
        assert executed == {
            "data": OrderedDict(
                [("verify", OrderedDict([("ok", True), ("verificationRate", 0.25)]))]
            )
        }
